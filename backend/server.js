...
// includes menu in context
function buildSiteContext({ products = [], menu = [], orders = [] }) {
    ...
    ? [
        ...
        ...products.map((p) => {
          const stock =
            p.inStock == null
              ? "inStock: unknown"
              : `inStock: ${p.inStock}${p.stockQty != null ? ` | stockQty: ${p.stockQty}` : ""}`;
          ...
          return `- ${p.title} | ${priceLine} | ${stock}${p.url ? ` | url: ${p.url}` : ""}\n  info: ${
            p.snippet ?? ""
          }`;
        }),
      ].join("\n")
  ...
  const menuContext = menu.length
    ? [
        ...
        ...menu.map((m) => {
          const stock = m.inStock == null ? "inStock: unknown" : `inStock: ${m.inStock}`;
          return `- ${m.title} | price: ${m.price ?? "N/A"} | rating: ${m.rating ?? "N/A"} | category: ${
            m.category ?? "N/A"
          } | tag: ${m.tag ?? "N/A"} | ${stock}\n  info: ${m.snippet ?? ""}`;
        }),
      ].join("\n")
  ...
  const ordersContext = orders.length
    ? [
        ...
        ...orders.map(
          (o) =>
            `- orderId: ${o.id} | status: ${o.status} | total: ${o.total ?? "N/A"} | items: ${
              o.itemCount ?? 0
            } | payment: ${o.paymentMethod ?? "N/A"} | createdAt: ${o.createdAt ?? "N/A"}`
        ),
      ].join("\n")
...
app.get("/api/chat/stream", async (req, res) => {
  ...
  try {
      ...
      input: [
        {
          ...
          content:
            ...
            "Use SITE CONTEXT as the source of truth for PRODUCTS and MENU ITEMS (prices, stock, descriptions), and order status. " +
    ...
    const msg = err?.message || err?.response?.data?.error?.message || "Unknown error";
