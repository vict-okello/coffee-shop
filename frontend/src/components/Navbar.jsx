...
function Navbar() {
  ...
  const { cart, cartCount, increaseQty, decreaseQty, removeFromCart, showCart, setShowCart } = useCart();
  ...
  return (
    <header className="sticky top-0 z-50 shadow-md w-full bg-[#7C573C]">
      <nav className="max-w-12xl mx-auto flex justify-between items-center px-6 py-4 relative">
        ...
        {/* Logo */}
        <NavLink to="/">
          <img src="/elizalogo.png" alt="logo" className="w-12 h-12 rounded-full" />
        </NavLink>
        ...
        <div className="hidden md:flex items-center gap-4">
          <CiUser size={28} className="text-black hover:text-white cursor-pointer transition-colors" />

          {/* Cart */}
          <div className="relative">
            <BsCart3 size={28} className="text-black hover:text-white cursor-pointer transition-colors" onClick={() => setShowCart(!showCart)} />
            {cartCount > 0 && (
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="flex justify-between items-center border-b py-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => removeFromCart(item.id)} className="text-red-500 px-2 py-1">Remove</button>
                        </div>
                      </div>
                    ))}
